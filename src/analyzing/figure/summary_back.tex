...
\node[result] (PROGWCET) [above right=of WCETANA,yshift=-20pt]
   {
      \begin{tabular}{@{}c@{}}
         WCET of\\
         Programs
...
\node[auxresult] (SLOWFACTOR) [below right=of WCETANA,yshift=20pt]
   {
      \begin{tabular}{@{}c@{}}
         Slowdown\\
         Factors
...
\node[interresult,node distance=2cm and 0.5cm] (ISOLWCET) [below right=of WCETANA]
   {
         In Isolation
   };
...
\path[draw,->] ([xshift=20pt]WCETANA) -- (ISOLWCET);
\path[draw,->] ([xshift=-20pt]WCETANA) -- ([xshift=-20pt]NOSHAREDWCET);
\path[draw,->] (ISOLWCET) -- (IMPACTWCET);
